...
for i in lst:
 ...
 num_emp = int(i.split(':')[1])elif i.split(':')[1] != '': costs[i.split(':')[0]] = int(i.split(':')[1]) costs = dict(sorted(costs.items(), key=lambda item: item[1]))
 map_keys = {}i = 1
...
min_cst = costs[map_keys[i+num_emp-1]] - costs[map_keys[i]]
 min_i = i
f = open("sample_output.txt", "w")f.write("The goodies selected for distribution are:\n")
